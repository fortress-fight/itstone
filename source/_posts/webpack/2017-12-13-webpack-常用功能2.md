...
2. 安装 loader `cnpm i -D file-loader url-loader`
    ...
    `url-loader` 用于处理入口文件中的链接，已经包含 `file-loader` 的功能可以直接将链接到的文件处理成 webpack 能够识别的模块
...
5. 解决问题
    ...
    可以使用当前 IP 作为 publicPath 然后在 dist 文件中启用服务，如果希望在本地运行可以使用 `path.resolve(__dirname, 'dist') + '/'`
